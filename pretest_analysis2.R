...
dotplot(ranef(stzmod))
dotplot(ranef(chzmod))
dotplot(ranef(viszmod))
summary(viszmod)
...
summary(lm(status~choice,longdim))
summary(stchmod)
performance(stchmod)
fixef(stchmod)
ranef(stchmod)
stvismod <- lmer(status ~ visibility  + (visibility|targetnames) + (visibility|ID),REML =FALSE,data = longdim)
confint.merMod(stchmod)
summary(stvismod)
dotplot(ranef(stvismod))
chvismod <- lmer(choice ~ visibility  + (visibility|targetnames) + (visibility|ID),REML =FALSE,data = longdim)
summary(chvismod)
plot(ranef.mer(chvismod))
performance(lm(status~choice,data = longdim))
nlme::intervals.lme(stvismod)
ranstch<-ranef(stchmod)
confint(stchmod, oldNames=FALSE)


by(longdim$belief,longdim$ID,sd)